...
export default function HeeraGrandDashboard() {
  ...
  const load = async () => {
    ...
    const [{ data: b }, { data: r }] = await Promise.all([
      supabase.from("bookings").select("*").eq("property", "heera").order("created_at", { ascending: false }),
      supabase.from("service_requests").select("*").eq("property", "heera").order("created_at", { ascending: false }),
    ]);
    setBookings(b || []);
    setRequests(r || []);
    setLoading(false);
      ...
      <main className="flex-1 p-6 overflow-auto">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white">Heera Grand</h1>
        ...
        <div className="flex gap-2 mb-4">
          {(["bookings", "requests"] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`px-4 py-2 rounded-lg text-sm font-semibold capitalize transition-colors ${tab === t ? "bg-amber-500 text-white" : "bg-gray-800 text-gray-400 hover:bg-gray-700"}`}>
                ...
                <tbody className="divide-y divide-gray-700/30">
                  {bookings.map(b => (
                    <tr key={b.id} className="hover:bg-gray-700/30">
                      <td className="py-3 px-4 text-amber-400 text-xs font-mono font-bold">{b.ref}</td>
